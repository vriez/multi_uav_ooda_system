...
class WorkloadBalancer:
    ...
    def compute_zone_contour(  # noqa: C901
        ...
        if mode == "grouped" and len(zone_ids) > 1:
            ...
            for zid in ordered_zones:
                ...
                zone_waypoints = self._generate_pattern_waypoints(
                    pattern, cx, cy, size, size
                )
    ...
    def _group_zones_spatially(self, tasks, num_uavs):
        ...
        for uav_idx in range(num_uavs):
            ...
            if uav_idx == 0:
                ...
                prev_centers = [
                    pos for zid, pos in zone_list if zid in zone_groups[uav_idx - 1]
                ]
                if prev_centers:
                    prev_center = np.mean(prev_centers, axis=0)
                    distances = [
                        np.linalg.norm(pos - prev_center)
                        for zid, pos in remaining_zones
                    ]
            ...
            while len(zone_groups[uav_idx]) < target_count and remaining_zones:
                ...
                distances = [
                    np.linalg.norm(pos - current_center) for zid, pos in remaining_zones
                ]
                ...
                # Update group center
                group_positions = [
                    pos for zid, pos in zone_list if zid in zone_groups[uav_idx]
                ]
    ...
    def _point_in_zones(self, x, y, zone_bounds, margin=5):
        ...
        for zb in zone_bounds:
            if (
                zb["min_x"] - margin <= x <= zb["max_x"] + margin
                and zb["min_y"] - margin <= y <= zb["max_y"] + margin
            ):
                return True
        return False
    ...
    def _generate_grouped_pattern(  # noqa: C901
        ...
        if pattern == "lawnmower":
            ...
            return self._generate_grouped_pattern(
                "perimeter", zone_bounds, min_x, max_x, min_y, max_y
            )
    ...
    def _order_zones_nearest(self, zone_centers):
        ...
        while remaining:
            ...
            nearest = min(
                remaining,
                key=lambda zid: (zone_centers[zid][0] - current_pos[0]) ** 2
    ...
    def assign_initial_workload(self, uavs, tasks):
        ...
        for i, uav_id in enumerate(uav_list):
            if i < len(zone_groups):
                ...
                logger.info(
                    f"{uav_id} assigned zones {assigned} ({['priority' if tasks[z].get('priority', 1) > 1 else 'normal' for z in assigned]}) with {len(contour_waypoints)} waypoints"
                )
    ...
    def reassign_recovered_uav(self, uav_id, uavs, tasks):  # noqa: C901
        ...
        for zid, task in tasks.items():
            ...
            if isinstance(zid, str) and (
                zid.startswith("pkg_") or zid.startswith("zone_")
            ):
                continue

            active_uavs = [
                uid
                for uid in task.get("assigned_uavs", [])
                if uid in uavs
                and uavs[uid]["operational"]
        ...
        if unassigned_zones:
            ...
            operational_uavs = [
                ...
                if u["operational"]
                and u["state"] in ["deploying", "patrolling"]
                and len(u["assigned_zones"]) > 1
            ]

            if operational_uavs:
                heaviest_uav = max(
                    operational_uavs, key=lambda uid: len(uavs[uid]["assigned_zones"])
                )

                # Take half of their zones (at least 1)
                zones_to_steal = uavs[heaviest_uav]["assigned_zones"][
                    : len(uavs[heaviest_uav]["assigned_zones"]) // 2 + 1
                ]
                ...
                log_message = (
                    f"{uav_id} stole zones {zones_to_steal} from {heaviest_uav}"
                )
            else:
                # Fallback: just take the least covered zone
                sorted_zones = sorted(
                    [z for z in tasks.keys() if tasks[z].get("type") != "asset"],
                ...
                if not sorted_zones:
                    ...
                    for old_uav_id in tasks[target_zone].get("assigned_uavs", []):
                        if (
                            old_uav_id in uavs
                            and old_uav_id != uav_id
                            and target_zone
                            in uavs[old_uav_id].get("assigned_zones", [])
                        ):
                            uavs[old_uav_id]["assigned_zones"].remove(target_zone)

                    uavs[uav_id]["assigned_zones"] = [target_zone]
                    tasks[target_zone]["assigned_uavs"] = [
                        uav_id
                    ]  # Replace, not append
        ...
        uavs[uav_id]["contour_waypoints"] = self.compute_zone_contour(
            uavs[uav_id]["assigned_zones"], tasks
        )
        uavs[uav_id]["waypoint_idx"] = 0
        ...
        for failed_zone in sorted_zones:
            ...
            if candidates:
                ...
                for old_uav_id in tasks[failed_zone].get("assigned_uavs", []):
                    if old_uav_id in uavs and failed_zone in uavs[old_uav_id].get(
                        "assigned_zones", []
                    ):
                ...
                new_contour = self.compute_zone_contour(
                    uavs[best_uav]["assigned_zones"], tasks
                )
            ...
            for uav_id in sorted(uavs.keys()):
                ...
                if uav.get("assigned_task"):
                    ...
                    if tasks and task_id in tasks:
                        ...
                        assignments.append(
                            f"{uav_id} {icon} Pkg#{task_id} | {desc} | Total:{delivered}"
                        )
                    else:
                    ...
                    delivered = uav.get("packages_delivered", 0)
                    assignments.append(
                        f"{uav_id} → IDLE (waiting) | Delivered:{delivered}"
                    )
            ...
            for uav_id in sorted(uavs.keys()):
                ...
                if uav["assigned_zones"]:
                    ...
                    if tasks and scenario_type == "search_rescue":
                        has_priority = any(
                            tasks.get(z, {}).get("priority", 1) > 1
                            for z in uav["assigned_zones"]
                        )
                        if has_priority:
    ...
    def ensure_full_coverage(self, uavs, tasks):  # noqa: C901
        ...
        logger.info(
            f"Found orphaned zones: {orphaned_zones}, distributing to {operational_uavs}"
        )
        ...
        for zid in orphaned_zones:
            ...
            for old_uav_id in tasks[zid].get("assigned_uavs", []):
                if old_uav_id in uavs and zid in uavs[old_uav_id].get(
                    "assigned_zones", []
                ):
        ...
        for uid in operational_uavs:
            if uavs[uid]["assigned_zones"]:
                uavs[uid]["contour_waypoints"] = self.compute_zone_contour(
                    uavs[uid]["assigned_zones"], tasks
                )
...
def init_scenario(scenario, custom_home=None):  # noqa: C901
    ...
    if scenario in ["surveillance", "search_rescue"]:
        ...
        if num_packages >= num_uavs:
            ...
            for i, (pkg_id, pkg) in enumerate(package_list[:num_uavs]):
                ...
                logger.info(
                    f"{uav_id} pre-assigned package {pkg_id} (priority {pkg['priority']:.2f})"
                )

            # Remaining packages stay as 'pending' for dynamic assignment
            remaining = num_packages - num_uavs
            logger.info(
                f"Delivery: {num_uavs} packages initially assigned, {remaining} packages pending for dynamic assignment"
            )
        else:
            ...
            logger.info(
                f"Delivery: {num_packages} packages assigned, {len(idle_uavs)} UAVs sent home"
            )

    safe_emit(
        "workload_update",
        {
            "assignments": workload_balancer.get_current_assignments(
                uavs, tasks, scenario_type
            )
        },
    )
    ...
    logger.info(
        f"All {cfg['uavs']} UAVs deployed from {home_base[:2]} for {scenario} mission"
    )
...
def simulation_loop():  # noqa: C901
    ...
    COVERAGE_DECAY_RATE = (
        0.5  # Coverage decays 0.5% per simulated second when unpatrolled
    )
        ...
        if mission_active:
            ...
            for uid, uav in uavs.items():
                ...
                if uav["battery"] <= 15 and not uav["returning"] and uav["operational"]:
                    ...
                    emit_ooda(
                        "observe",
                        f"{uid} returning to base (low battery)",
                        critical=False,
                    )
                    ...
                    if abandoned_zones:
                        ...
                        if operational_uavs:
                            ...
                            if assignments:
                                ...
                                emit_ooda(
                                    "decide",
                                    f"OODA #{ooda_count}: Redistributed zones {abandoned_zones} from {uid}",
                                    critical=False,
                                )
                ...
                if uav["returning"]:
                    ...
                    logger.debug(
                        f"{uid} RETURNING: position={uav['position'][:2]}, distance_to_home={dist:.1f}m, battery={uav['battery']:.1f}%, operational={uav['operational']}"
                    )
                    ...
                    if dist > HOME_ARRIVAL_THRESHOLD:
                        ...
                        if uav["battery"] <= 0:
                            ...
                            emit_ooda(
                                "observe",
                                f"{uid} CRASHED during return (battery exhausted)",
                                critical=True,
                            )
                    else:
                        ...
                        logger.debug(
                            f"{uid} charging: battery={uav['battery']:.1f}%, operational={uav['operational']}, state={uav['state']}"
                        )
                        ...
                        if uav["battery"] >= RECOVERY_THRESHOLD:
                            ...
                            uav["operational"] = (
                                True  # Set operational when fully charged
                            )
                            logger.info(
                                f"{uid} finished charging (battery: {uav['battery']:.1f}%), state='{uav['state']}', operational=True, awaiting OODA reassignment"
                            )
                    continue
                ...
                if (
                    dist_to_home < HOME_PROXIMITY_THRESHOLD
                    and uav["battery"] < RECOVERY_THRESHOLD
                    and uav["state"] not in ["patrolling", "deploying"]
                ):
                    uav["returning"] = True
                    uav["state"] = "charging"
                    uav["operational"] = False
                    ...
                    uav["position"][1] = home_base[1]
                    logger.warning(
                        f"{uid} failsafe: forcing charging state (battery: {uav['battery']:.1f}%)"
                    )
                    continue
                ...
                if uav["state"] in ["recovered", "charging", "idle"]:
                    ...
                    if dist_to_home < HOME_ARRIVAL_THRESHOLD:
                        ...
                        if uav["battery"] < RECOVERY_THRESHOLD:
                            ...
                            if uav["state"] != "charging":
                                uav["state"] = "charging"
                                logger.info(
                                    f"{uid} started charging at home (battery: {uav['battery']:.1f}%)"
                                )
                        ...
                        # Battery >= RECOVERY_THRESHOLD: ensure UAV is in correct recovered state
                            ...
                            logger.info(
                                f"{uid} charging complete, state set to '{uav['state']}' (battery: {uav['battery']:.1f}%)"
                            )
                    ...
                    if scenario_type != "delivery" and uav["state"] in [
                        "recovered",
                        "charging",
                    ]:
                        continue
                ...
                if uav["state"] == "returning_to_grid":
                    ...
                    if uav.get("return_to_grid_target") is None:
                        ...
                        logger.info(
                            f"{uid} calculated grid return target: ({target[0]:.1f}, {target[1]:.1f})"
                        )
                    ...
                    if dist > WAYPOINT_ARRIVAL_THRESHOLD:
                        ...
                        if uav["battery"] <= 0:
                            ...
                            uav["state"] = "crashed"
                            logger.error(
                                f"{uid} crashed while returning to grid (battery exhausted)"
                            )
                            emit_ooda(
                                "observe",
                                f"{uid} CRASHED returning to grid (battery exhausted)",
                                critical=True,
                            )
                    else:
                        ...
                        emit_ooda(
                            "act",
                            f"{uid} returned to grid - resuming patrol",
                            critical=False,
                        )
                ...
                if uav["state"] == "searching" and (
                    uav["searching_asset"] or uav["guardian_of_asset"]
                ):
                    ...
                    if asset_id in tasks:
                        ...
                        distance_to_real_asset = np.linalg.norm(
                            real_asset_pos - uav_pos
                        )

                        # Refine search area as UAV gets closer (narrowing uncertainty)
                        # But don't give exact position until pinpointed
                        if (
                            distance_to_real_asset <= SAR_VISIBILITY_RADIUS
                            and not asset["pinpointed"]
                        ):
                            # Update with refined estimate (less noise as we get closer)
                            uncertainty = max(
                                2, distance_to_real_asset * 0.1
                            )  # Reduce uncertainty as we approach
                            noise = np.random.uniform(-uncertainty, uncertainty, 2)
                            asset["last_known_position"] = (
                                asset["position"].copy() + np.append(noise, [0])
                            ).tolist()
                            asset_pos = np.array(asset["last_known_position"][:2])

                        # SAFETY CHECK: Last known position outside grid requires permission
                        asset_outside_grid = is_position_outside_grid(
                            asset["last_known_position"]
                        )
                        permission_granted = uav.get(
                            "permission_granted_for_target"
                        ) == tuple(asset_pos)

                        if asset_outside_grid and not permission_granted:
                            # Asset is outside grid - stop at boundary and request permission
                            boundary_pos = calculate_boundary_intersection(
                                uav["position"][:2], asset_pos
                            )
                            boundary_dist = np.linalg.norm(
                                np.array(boundary_pos) - np.array(uav["position"][:2])
                            )
                            ...
                            if boundary_dist <= WAYPOINT_ARRIVAL_THRESHOLD:
                                ...
                                if not uav.get("awaiting_permission"):
                                    ...
                                    uav["boundary_stop_position"] = (
                                        boundary_pos.tolist()
                                        if hasattr(boundary_pos, "tolist")
                                        else list(boundary_pos)
                                    )
                                    uav["out_of_grid_target"] = list(
                                        asset["last_known_position"]
                                    )
                                    uav["state"] = "awaiting_permission"
                                    logger.warning(
                                        f"{uid} stopped at boundary - {asset_id} last known at ({asset['last_known_position'][0]:.1f}, {asset['last_known_position'][1]:.1f}) is outside grid"
                                    )
                                    emit_ooda(
                                        "observe",
                                        f"{uid} BOUNDARY STOP - {asset_id} outside safe zone. Double-click UAV to grant permission.",
                                        critical=True,
                                    )
                                ...
                                if uav["battery"] <= 0:
                                    ...
                                    logger.error(
                                        f"{uid} battery depleted while awaiting permission at boundary"
                                    )
                                    emit_ooda(
                                        "observe",
                                        f"{uid} CRASHED at boundary (battery exhausted)",
                                        critical=True,
                                    )
                                ...
                                boundary_dist_norm = np.sqrt(
                                    boundary_dx**2 + boundary_dy**2
                                )
                                speed_factor = BASE_CRUISE_SPEED
                                uav["position"][0] += (
                                    (boundary_dx / boundary_dist_norm)
                                    * speed_factor
                                    * dt
                                )
                                uav["position"][1] += (
                                    (boundary_dy / boundary_dist_norm)
                                    * speed_factor
                                    * dt
                                )
                                uav["position"][2] = 15
                        ...
                        num_circle_points = 8
                        if (
                            not uav["circle_waypoints"]
                            or len(uav["circle_waypoints"]) == 0
                        ):
                        ...
                        if uav["circle_waypoints"]:
                            ...
                            if dist < WAYPOINT_ARRIVAL_THRESHOLD:
                                ...
                                if (
                                    new_idx == 0
                                    and waypoint_idx == len(uav["circle_waypoints"]) - 1
                                ):
                                    uav["circle_count"] += 1
                                    logger.info(
                                        f"{uid} completed circle #{uav['circle_count']} around {asset_id}"
                                    )

                                    # After 3 circles, asset is identified - return to grid
                                    if (
                                        uav["circle_count"] >= 3
                                        and uav["guardian_of_asset"] is None
                                    ):
                                        # Mark asset as identified (no guardian needed - just identify and move on)
                                        asset["identified"] = True
                                        asset["rescued"] = (
                                            True  # Consider it rescued after identification
                                        )

                                        # Check if UAV is outside grid - need to return first
                                        uav_outside_grid = is_position_outside_grid(
                                            uav["position"]
                                        )
                                        ...
                                        if uav_outside_grid:
                                            ...
                                            uav["circle_count"] = 0
                                            uav["state"] = (
                                                "returning_to_grid"  # New state for returning to grid
                                            )
                                            uav["return_to_grid_target"] = (
                                                None  # Will calculate boundary point
                                            )
                                            ...
                                            logger.info(
                                                f"{uid} identified {asset_id} outside grid - returning to grid boundary"
                                            )
                                            emit_ooda(
                                                "orient",
                                                f"{uid} identified {asset_id} - returning to grid",
                                                critical=False,
                                            )

                                            # Update mission metrics
                                            if not mission_metrics.get(
                                                f"{asset_id}_counted", False
                                            ):
                                                mission_metrics["assets_rescued"] = (
                                                    mission_metrics.get(
                                                        "assets_rescued", 0
                                                    )
                                                    + 1
                                            ...
                                            uav["circle_count"] = 0
                                            uav["state"] = (
                                                "recovered"  # Set to recovered so it gets reassigned zones
                                            )
                                            ...
                                            logger.info(
                                                f"{uid} identified {asset_id} inside grid - resuming patrol"
                                            )
                                            emit_ooda(
                                                "orient",
                                                f"{uid} identified {asset_id} - resuming patrol",
                                                critical=False,
                                            )
                                    ...
                                    # Guardian monitors for limited time (3 seconds)
                                        ...
                                        guardian_duration = time.time() - uav.get(
                                            "guardian_start_time", time.time()
                                        )

                                        # Release guardian after 3 seconds
                                        if guardian_duration >= 3.0:
                                            logger.info(
                                                f"{uid} completed guardian duty for {asset_id} ({guardian_duration:.1f}s) - returning to patrol"
                                            )
                                            emit_ooda(
                                                "orient",
                                                f"{uid} completed monitoring {asset_id} - returning to patrol",
                                                critical=False,
                                            )
                                            ...
                                            uav["guardian_start_time"] = None
                                            uav["state"] = (
                                                "recovered"  # Get reassigned to patrol zones
                                            )
                                ...
                                speed_factor = (
                                    BASE_CRUISE_SPEED * 0.8
                                )  # Slightly slower when searching
                                uav["position"][0] += (dx / dist) * speed_factor * dt
                                uav["position"][1] += (dy / dist) * speed_factor * dt
                                uav["position"][2] = 15
                            ...
                            if uav["battery"] <= 0:
                                ...
                                emit_ooda(
                                    "observe",
                                    f"{uid} crashed while searching (battery exhausted)",
                                    critical=True,
                                )
                    continue
                ...
                if (
                    not uav["assigned_zones"]
                    or "contour_waypoints" not in uav
                    or not uav["contour_waypoints"]
                ):
                    uav["state"] = "idle"
                    continue
                ...
                if dist < WAYPOINT_ARRIVAL_THRESHOLD:
                    ...
                    if next_idx == 0:
                        ...
                        for zone_id in uav["assigned_zones"]:
                            if zone_id in tasks:
                                tasks[zone_id]["coverage"] = min(
                                    100, tasks[zone_id]["coverage"] + 20
                                )
                else:
                ...
                if uav["battery"] <= 0:
                    ...
                    uav["contour_waypoints"] = []
                    emit_ooda(
                        "observe", f"{uid} crashed (battery exhausted)", critical=True
                    )
            ...
            if scenario_type == "surveillance" or scenario_type == "search_rescue":
                ...
                for zid, task in tasks.items():
                    ...
                    if not active_uavs:
                        ...
                        task["coverage"] = max(
                            0, task["coverage"] - COVERAGE_DECAY_RATE * dt
                        )
                ...
                if scenario_type == "search_rescue":
                    ...
                    for asset_id, asset in tasks.items():
                        ...
                        for uav_id, uav in uavs.items():
                            ...
                            if (
                                distance <= SAR_VISIBILITY_RADIUS
                                and uav["searching_asset"] is None
                            ):
                                if uav_id not in asset["searching_uavs"] and uav[
                                    "state"
                                ] in ["patrolling", "deploying", "searching"]:
                                    ...
                                    if is_first_contact:
                                        ...
                                        logger.info(
                                            f"{uav_id} FIRST CONTACT with {asset_id} in area {coord_str}"
                                        )
                                        emit_ooda(
                                            "observe",
                                            f"{uav_id} - contact visual in area {coord_str}",
                                            critical=False,
                                        )
                                    else:
                                        # Additional UAVs confirming the contact
                                        logger.info(
                                            f"{uav_id} CONFIRMING {asset_id} in area {coord_str}"
                                        )
                                        emit_ooda(
                                            "observe",
                                            f"{uav_id} - confirming area {coord_str}",
                                            critical=False,
                                        )
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    if (
                                        asset["guardian_uav"]
                                        and asset["guardian_uav"]
                                        not in asset["detected_by"]
                                    ):
                                        asset["detected_by"].append(
                                            asset["guardian_uav"]
                                        )

                                    logger.info(
                                        f"{asset_id} PINPOINTED with radius {asset['search_radius']:.1f}m!"
                                    )
                                    emit_ooda(
                                        "observe",
                                        f"{asset_id.upper()} location pinpointed!",
                                        critical=False,
                                    )
                        ...
                        if asset["guardian_uav"] is not None and not asset["rescued"]:
                            ...
                            emit_ooda(
                                "orient",
                                f'{asset_id.upper()} SECURED! Guardian {asset["guardian_uav"]} assigned (t={asset["rescue_time"]:.1f}s)',
                                critical=False,
                            )
                            logger.info(
                                f"{asset_id} secured with guardian {asset['guardian_uav']} after {asset['rescue_time']:.1f}s"
                            )

                    # Check mission completion - all assets have guardians assigned
                    if mission_metrics["assets_rescued"] >= mission_metrics.get(
                        "total_assets", 0
                    ):
                ...
                for uid, uav in uavs.items():
                    ...
                    if uav["assigned_task"] is None and uav["state"] in [
                        "deploying",
                        "idle",
                        "patrolling",
                    ]:
                        ...
                        if pending_tasks:
                            ...
                            logger.info(
                                f"{uid} dynamically assigned package {task_id} (priority {task['priority']:.2f}, {len(pending_tasks)-1} packages remaining)"
                            )
                        else:
                    ...
                    if uav["assigned_task"] is not None:
                        ...
                        if uav["awaiting_permission"]:
                            ...
                            if uav["battery"] <= BATTERY_LOW_THRESHOLD and not uav.get(
                                "battery_warning"
                            ):
                                uav["battery_warning"] = True
                                emit_ooda(
                                    "observe",
                                    f'{uid} LOW BATTERY ({uav["battery"]:.0f}%) while awaiting permission',
                                    critical=True,
                                )
                                logger.warning(
                                    f"{uid} low battery warning while awaiting permission: {uav['battery']:.1f}%"
                                )
                        ...
                        if dist > DELIVERY_ARRIVAL_THRESHOLD:
                            ...
                            if (
                                ...
                                if boundary_dist <= DELIVERY_ARRIVAL_THRESHOLD:
                                    ...
                                    logger.warning(
                                        f"{uid} stopped at grid boundary ({boundary_pos[0]:.1f}, {boundary_pos[1]:.1f}) - {target_type} destination ({target[0]:.1f}, {target[1]:.1f}) is outside grid. Awaiting permission."
                                    )
                                    emit_ooda(
                                        "observe",
                                        f"{uid} BOUNDARY STOP - {target_type} at ({target[0]:.0f}, {target[1]:.0f}) is outside safe zone. Double-click UAV to grant permission.",
                                        critical=True,
                                    )
                                else:
                                    # Move toward boundary
                                    speed_factor = BASE_CRUISE_SPEED
                                    uav["position"][0] += (
                                        (boundary_dx / boundary_dist)
                                        * speed_factor
                                        * dt
                                    )
                                    uav["position"][1] += (
                                        (boundary_dy / boundary_dist)
                                        * speed_factor
                                        * dt
                                    )
                                    uav["position"][2] = 10
                            ...
                            if uav["battery"] <= 0:
                                ...
                                if task_id in tasks:
                                    ...
                                    task["visible"] = (
                                        True  # Make package visible again for reassignment
                                    )
                                    logger.error(
                                        f"{uid} CRASHED during delivery - {task_id} reset to pending for reassignment"
                                    )
                                    emit_ooda(
                                        "observe",
                                        f"{uid} CRASHED - package {task_id} will be reassigned",
                                        critical=True,
                                    )
                                else:
                                    logger.error(
                                        f"{uid} battery depleted during delivery - mission failed"
                                    )

                                uav["assigned_task"] = None
                                uav["carrying_package"] = (
                                    False  # UAV drops package when crashing
                                )
                                uav["awaiting_permission"] = False
                                uav["permission_granted_for_target"] = None
                            ...
                            if uav["battery"] <= BATTERY_LOW_THRESHOLD and not uav.get(
                                ...
                                emit_ooda(
                                    "observe",
                                    f'{uid} LOW BATTERY ({uav["battery"]:.0f}%) during delivery',
                                    critical=True,
                                )
                                logger.warning(
                                    f"{uid} low battery warning during delivery: {uav['battery']:.1f}%"
                                )
                        else:
                            # Arrived at target
                            if task["status"] == "assigned":
                                ...
                                uav["carrying_package"] = (
                                    True  # Mark UAV as carrying package
                                )
                                ...
                                task["visible"] = (
                                    True  # Show package at dropoff location (optional, can remove if not needed)
                                )
                                uav["carrying_package"] = (
                                    False  # UAV no longer carrying package
                                )
                                ...
                                emit_ooda(
                                    "act",
                                    f'{uid} completed delivery {task_id} ({mission_metrics["deliveries_completed"]}/{mission_metrics["total_packages"]})',
                                    critical=False,
                                )
                                logger.info(
                                    f"{uid} delivered package {task_id} ({mission_metrics['deliveries_completed']}/{mission_metrics['total_packages']} complete) - now idle, ready for next assignment"
                                )

                                # Check if all deliveries complete - send all UAVs home
                                if (
                                    mission_metrics["deliveries_completed"]
                                    >= mission_metrics["total_packages"]
                                ):
                                    ...
                                    if not mission_metrics.get(
                                        ...
                                        emit_ooda(
                                            "act",
                                            f" ALL DELIVERIES COMPLETE - Sending all UAVs home",
                                            critical=False,
                                        )
                                        logger.info(
                                            "All deliveries completed - returning all UAVs to base"
                                        )

                                        # Send all operational UAVs home
                                        for uav_id, u in uavs.items():
                                            if u["operational"] and u["state"] not in [
                                                "returning",
                                                "charging",
                                                "crashed",
                                            ]:
                                                ...
                                                u["returning"] = True
                                                logger.info(
                                                    f"{uav_id} returning to base after mission completion"
                                                )
            ...
            if scenario_type in ["search_rescue", "delivery"]:
                ...
                if scenario_type == "search_rescue":
                    ...
                    objectives_complete = mission_metrics[
                        "assets_rescued"
                    ] >= mission_metrics.get("total_assets", 0)

                    # Only announce completion once
                    if (
                        objectives_complete
                        and mission_active
                        and not mission_metrics.get("completion_announced", False)
                    ):
                        ...
                        emit_ooda(
                            "orient",
                            f' All {mission_metrics["total_assets"]} known assets secured! Guardians monitoring, other UAVs continue area patrol (t={elapsed:.1f}s)',
                            critical=False,
                        )
                        logger.info(
                            f"SAR objectives complete: all known assets have guardians - continuing patrol"
                        )
                    ...
                    ] >= mission_metrics.get("total_packages", 0)
                    final_return_initiated = mission_metrics.get(
                        "final_return_initiated", False
                    )
                    ...
                    if objectives_complete and final_return_initiated:
                        ...
                        all_home = all(
                            u["state"] in ["charging", "crashed"] for u in uavs.values()
                        )
                        ...
                        if all_home and mission_active:
                            ...
                            emit_ooda(
                                "act",
                                f" DELIVERY MISSION COMPLETE - All UAVs returned home (t={elapsed:.1f}s) - Mission stopped",
                                critical=False,
                            )
                            logger.info(
                                f"DELIVERY mission auto-stopped: objectives complete and all UAVs home"
                            )
                            mission_active = False
            ...
            if (
                scenario_type in ["surveillance", "search_rescue"]
                and current_time - last_reassignment_time
                > REASSIGNMENT_INTERVAL / simulation_speed
            ):
                ...
                if coverage_changed:
                    ...
                    emit_ooda(
                        "decide",
                        f"OODA #{ooda_count}: Fixed orphaned zones",
                        critical=False,
                    )
                    safe_emit(
                        "workload_update",
                ...
                for uid, u in uavs.items():
                    logger.debug(
                        f"  {uid}: state={u['state']}, operational={u['operational']}, assigned_zones={u.get('assigned_zones', [])}, searching={u.get('searching_asset')}, guardian={u.get('guardian_of_asset')}"
                    )
                ...
                if ready_uavs:
                    logger.info(
                        f"OODA: Found {len(ready_uavs)} UAVs ready for assignment: {ready_uavs}"
                    )
                    new_assignments = []
                    for uid in sorted(ready_uavs):
                        logger.info(
                            f"OODA: Reassigning {uid} (battery={uavs[uid]['battery']:.1f}%, state={uavs[uid]['state']})"
                        )
                        assignments = workload_balancer.reassign_recovered_uav(
                            uid, uavs, tasks
                        )
                        ...
                        emit_ooda(
                            "act",
                            f'{uid} redeployed to zones {uavs[uid]["assigned_zones"]} (battery: {uavs[uid]["battery"]:.0f}%)',
                            critical=False,
                        )
                    ...
                    emit_ooda(
                        "decide",
                        f"OODA #{ooda_count}: {len(ready_uavs)} UAVs deployed",
                        critical=False,
                    )
            ...
            if current_time - last_telemetry_time > TELEMETRY_INTERVAL:
                ...
                avg_coverage = (
                    np.mean([t["coverage"] for t in tasks_with_coverage])
                    if tasks_with_coverage
                    else 0
                )
                ...
                if scenario_type == "surveillance":
                    ...
                    completion = (
                        (mission_metrics["assets_rescued"] / total_assets * 100)
                        if total_assets > 0
                        else 0
                    )
                elif scenario_type == "delivery":
                    ...
                    completion = (
                        (mission_metrics["deliveries_completed"] / total_packages * 100)
                        if total_packages > 0
                        else 0
                    )
                else:
...
def handle_disconnect():
    ...
    try:
        ...
        logger.info(
            f"Client disconnected: {client_id} (Remaining: {len(connected_clients)})"
        )
    except Exception as e:
...
def handle_start(data):  # noqa: C901
    ...
    if scenario_type == "surveillance":
        if "patrol_pattern" in data:
            ...
            logger.info(
                f"Surveillance configuration: patrol_pattern={data['patrol_pattern']}"
            )
        if "pattern_mode" in data:
            SCENARIOS["surveillance"]["pattern_mode"] = data["pattern_mode"]
            logger.info(
                f"Surveillance configuration: pattern_mode={data['pattern_mode']}"
            )
    ...
    if scenario_type == "search_rescue":
        ...
        if "consensus_required" in data:
            SCENARIOS["search_rescue"]["consensus_required"] = int(
                data["consensus_required"]
            )
            logger.info(
                f"SAR configuration: consensus_required={data['consensus_required']}"
            )
    ...
    emit_ooda(
        "observe",
        f"Mission started: {scenario_type.upper()} scenario with {len(uavs)} UAVs",
        critical=False,
    )
    emit_ooda(
        "orient", f"Analyzing mission parameters and fleet capabilities", critical=False
    )
    emit_ooda(
        "decide",
        f"Assigning initial tasks to {len(uavs)} operational UAVs",
        critical=False,
    )
    emit_ooda(
        "act", f"All {len(uavs)} UAVs deployed from {home_base[:2]}", critical=False
    )
...
def handle_pattern(data):
    ...
    emit_ooda(
        "decide", f"Patrol pattern changed to {new_pattern.upper()}", critical=False
    )
...
def handle_pattern_mode(data):
    ...
    mode_label = (
        "GROUPED (combined area)" if new_mode == "grouped" else "PER ZONE (individual)"
    )
...
def handle_failure(data):  # noqa: C901
    ...
    if uav_id == "random":
        operational = [
            uid
            for uid, u in uavs.items()
            if u["operational"] and not u["returning"] and u["assigned_zones"]
        ]
        if not operational:
            return
    ...
    operational = [
        uid
        for uid, u in uavs.items()
        if u["operational"] and u["state"] not in ["returning", "charging", "recovered"]
    ]
    ...
    if operational and failed_zones:
        ...
        if unassigned:
            ...
            for zone_id in unassigned:
                best_uav = min(
                    operational, key=lambda u: len(uavs[u]["assigned_zones"])
                )

                # Clear previous UAV assignments for this zone
                for old_uav_id in tasks[zone_id].get("assigned_uavs", []):
                    if old_uav_id in uavs and zone_id in uavs[old_uav_id].get(
                        "assigned_zones", []
                    ):
                ...
                new_contour = workload_balancer.compute_zone_contour(
                    uavs[best_uav]["assigned_zones"], tasks
                )
                uavs[best_uav]["contour_waypoints"] = new_contour
        ...
        emit_ooda(
            "decide",
            f"OODA #{ooda_count}: Redistributed {len(failed_zones)} zones",
            critical=False,
        )
...
def handle_manual_recovery(data):
    ...
    if uav_id in uavs and uavs[uav_id]["state"] == "crashed":
        ...
        emit_ooda(
            "act",
            f"{uav_id} manual recovery initiated. Returning to base.",
            critical=False,
        )
        safe_emit(
            "workload_update",
...
def handle_charge_battery(data):
    ...
    logger.info(
        f"CHARGE_BATTERY called for {uav_id}: {old_battery:.1f}% -> 100%, state={uav['state']}, searching_asset={uav.get('searching_asset')}"
    )
    ...
    if uav["state"] in ["returning", "charging"]:
        ...
        emit_ooda(
            "observe",
            f"{uav_id} battery charged to 100% - ready for redeployment",
            critical=False,
        )
        ...
        emit_ooda(
            "observe",
            f"{uav_id} battery topped up: {old_battery:.0f}% -> 100%",
            critical=False,
        )
...
def handle_charge_all():
    ...
    if charged_count > 0:
        emit_ooda(
            "observe", f"All {charged_count} UAVs charged to 100%", critical=False
        )
        safe_emit(
            "workload_update",
            {
...
def handle_drain_battery(data):
    ...
    emit_ooda(
        "observe", f"{uav_id} battery drained: {old_battery:.0f}% → 15%", critical=False
    )
    safe_emit(
        "workload_update",
        {
...
def handle_grant_permission(data):
    ...
    logger.info(
        f"GRANT_PERMISSION: {uav_id} at position {uav['position']}, battery {uav['battery']:.1f}%, state '{uav['state']}'"
    )

    # Check if UAV is in awaiting_permission state
    if not uav.get("awaiting_permission", False):
        logger.warning(
            f"Attempted to grant permission to {uav_id} which is not awaiting permission (state: {uav['state']})"
        )
        return
    ...
    logger.info(
        f"GRANT_PERMISSION: {uav_id} permission cleared, searching_asset={uav.get('searching_asset')}, assigned_task={uav.get('assigned_task')}"
    )
    ...
    if target:
        uav["permission_granted_for_target"] = tuple(
            target[:2]
        )  # Store as tuple for comparison
        logger.info(
            f"Permission granted for {uav_id} to proceed to out-of-grid destination ({target[0]:.1f}, {target[1]:.1f}) - resuming {mission_context}"
        )
        emit_ooda(
            "act",
            f"{uav_id} PERMISSION GRANTED - proceeding to out-of-grid {mission_context} ({target[0]:.0f}, {target[1]:.0f})",
            critical=False,
        )
    else:
        uav["permission_granted_for_target"] = None
        ...
        emit_ooda(
            "act", f"{uav_id} PERMISSION GRANTED - resuming delivery", critical=False
        )
...
def handle_move_asset(data):
    ...
    if asset_id in tasks and tasks[asset_id].get("type") == "asset":
        ...
        emit_ooda(
            "act",
            f"{asset_id} relocated: ({old_pos[0]:.0f},{old_pos[1]:.0f}) → ({new_x:.0f},{new_y:.0f})",
            critical=False,
        )
...
def handle_randomize_assets():
    ...
    if randomized_count > 0:
        emit_ooda(
            "act",
            f"Randomized {randomized_count} asset positions within grid",
            critical=False,
        )
        logger.info(
            f"Randomized {randomized_count} asset/package positions within grid boundaries"
        )
