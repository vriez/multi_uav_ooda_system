...
class QuadcopterDynamics:
    ...
    def state_derivative(self, state: np.ndarray, t: float, 
                        motor_speeds: np.ndarray) -> np.ndarray:
        """
...
class CascadePIDController:
    ...
    def compute_control(self, state: dict, setpoint: dict, dt: float) -> np.ndarray:
        ...
        acc_sp = (self.vel_kp * vel_error + 
                 self.vel_ki * self.vel_integral +
                 self.vel_kd * (vel_error - getattr(self, 'prev_vel_error', vel_error)) / dt)
