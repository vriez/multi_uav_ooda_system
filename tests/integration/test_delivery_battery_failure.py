...
class TestDeliveryBatteryFailure:
    ...
    def test_battery_drain_outside_grid_after_permission(self):
        ...
        print(f"  {uav_2_id} battery: {uav_2['battery']:.1f}% (better than {uav_id}: {uav['battery']:.1f}%)")
    ...
    def test_multiple_uavs_battery_failure_cascade(self):
        ...
        uavs = {
            'uav_1': {'battery': 20.0, 'state': 'idle', 'assigned_task': None, 'operational': True, 'returning': False},
            'uav_2': {'battery': 18.0, 'state': 'idle', 'assigned_task': None, 'operational': True, 'returning': False},
            'uav_3': {'battery': 90.0, 'state': 'idle', 'assigned_task': None, 'operational': True, 'returning': False}
        }
