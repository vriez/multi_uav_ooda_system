...
class TestDeliveryOODACharging:
    ...
    def test_immediate_assignment_after_charging(self):
        ...
        if uavs['uav_1']['battery'] >= RECOVERY_THRESHOLD:
            ...
            pending_packages = [(pid, p) for pid, p in packages.items()
                               if p['status'] == 'pending']
            ...
            if pending_packages:
                ...
                print(f"  ORIENT: Highest priority package is {task_id} (priority={task['priority']:.1f})")
        ...
        assert uavs['uav_1']['state'] == 'delivering', "UAV should be in delivering state"
        assert uavs['uav_1']['assigned_task'] == 'pkg_1', "Should be assigned highest priority package"
        assert packages['pkg_1']['status'] == 'assigned', "Package should be assigned"
        assert packages['pkg_1']['assigned_uav'] == 'uav_1', "Package should be assigned to uav_1"

        print("Verification:")
        print(f"  ✓ uav_1 state = {uavs['uav_1']['state']} (expected: delivering)")
        print(f"  ✓ uav_1 assigned_task = {uavs['uav_1']['assigned_task']} (expected: pkg_1)")
    ...
    def test_multiple_uavs_charging_sequential_assignment(self):
        ...
        packages = {
            f'pkg_{i}': {'status': 'pending', 'priority': 3.0 - i*0.5, 'assigned_uav': None}
            for i in range(1, 6)
        }

        uavs = {
            'uav_1': {'battery': 79.0, 'state': 'charging', 'operational': False, 'returning': True, 'assigned_task': None},
            'uav_2': {'battery': 70.0, 'state': 'charging', 'operational': False, 'returning': True, 'assigned_task': None},
            'uav_3': {'battery': 60.0, 'state': 'charging', 'operational': False, 'returning': True, 'assigned_task': None}
        }
        ...
        pending = [(p, packages[p]) for p in packages if packages[p]['status'] == 'pending']
        ...
        print(f"Remaining: {len(remaining_pending)} packages still pending (pkg_4, pkg_5)")
