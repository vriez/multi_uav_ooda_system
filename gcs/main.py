...
class GroundControlStation:
    ...
    def __init__(self, config_path: str):
        ...
        self.ooda_engine = OODAEngine(
            self.config, dashboard_bridge=None
        )  # Will be set later
    ...
    def _accept_connections(self):
        ...
        while self.running:
            try:
                ...
                # Handle registration
                threading.Thread(
                    target=self._handle_registration, args=(conn, addr), daemon=True
                ).start()
    ...
    def on_uav_failure(self, uav_id: int, failure_type: str):
        ...
        if self.dashboard_bridge:
            self.dashboard_bridge.notify_ooda_event(
                f"UAV {uav_id} FAILED: {failure_type}", is_critical=True
            )
        ...
        decision = self.ooda_engine.trigger_ooda_cycle(
            fleet_state, self.mission_db, self.constraint_validator
        )
    ...
    def _dispatch_mission_updates(self, reallocation_plan: Dict[int, list]):
        ...
        for uav_id, task_ids in reallocation_plan.items():
            ...
            # Send update command
            self._send_command(
                uav_id, "update_mission", {"waypoints": waypoints, "task_ids": task_ids}
            )
...
def main():
    ...
    try:
        ...
        while True:
            ...
            logger.info(
                f"Status: {status['fleet']['operational']} operational UAVs, "
