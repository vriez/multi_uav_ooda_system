...
class ConstraintValidator:
    ...
    def check_battery_constraint(self, uav_id: int, task_id: int,
        ...
        spare_capacity = available_energy - committed_energy - (self.battery_safety_reserve * battery_capacity_wh)
        
        if spare_capacity >= energy_required_wh:
            logger.debug(f"UAV {uav_id} battery OK for task {task_id}: "
                        f"{spare_capacity:.1f} Wh spare, {energy_required_wh:.1f} Wh needed")
            return True
        else:
            logger.debug(f"UAV {uav_id} insufficient battery for task {task_id}: "
                        f"{spare_capacity:.1f} Wh spare, {energy_required_wh:.1f} Wh needed")
            return False
            
    def check_payload_constraint(self, uav_id: int, task_id: int,
                                 fleet_state, mission_db) -> bool:
        """
        ...
        if available_payload >= required_payload:
            logger.debug(f"UAV {uav_id} payload OK for task {task_id}: "
                        f"{available_payload:.1f} kg available, {required_payload:.1f} kg needed")
            return True
        else:
            logger.debug(f"UAV {uav_id} insufficient payload for task {task_id}: "
                        f"{available_payload:.1f} kg available, {required_payload:.1f} kg needed")
            return False
            
    def check_time_constraint(self, uav_id: int, task_id: int,
                             fleet_state, mission_db) -> bool:
        """
        ...
        if time_available >= total_time:
            logger.debug(f"UAV {uav_id} time OK for task {task_id}: "
                        f"{time_available:.0f}s available, {total_time:.0f}s needed")
            return True
        else:
            logger.debug(f"UAV {uav_id} insufficient time for task {task_id}: "
                        f"{time_available:.0f}s available, {total_time:.0f}s needed")
            return False
            
    def _estimate_committed_energy(self, uav_id: int, task_ids: list,
                                   uav_position: np.ndarray, mission_db) -> float:
    ...
    def check_collision_avoidance(self, uav_id: int, waypoints: list,
                                  fleet_state, mission_db) -> bool:
        """
        ...
        for other_id in fleet_state.operational_uavs:
            ...
            for wp in waypoints:
                ...
                if distance < safety_buffer:
                    logger.warning(f"Collision risk between UAV {uav_id} and {other_id}: "
