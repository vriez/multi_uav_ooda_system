...
Key modules: \texttt{gcs/ooda\_engine.py} (phase timing), \texttt{gcs/fleet\_monitor.py} (failure detection), \texttt{gcs/constraint\_validator.py} (safety margins), \texttt{gcs/mission\_manager.py} (task tracking), \texttt{gcs/objective\_function.py} (optimization), \texttt{uav/simulation.py} (dynamics), \texttt{uav/client.py} (communication). Source publicly available (EULÁLIO REIS, 2025).
...
The web dashboard renders fleet state via Flask/WebSocket: UAV positions, battery bars (2 Hz, 20\% reserve marked), OODA timeline, and alert log. Post-mission analysis generates coverage traces and efficiency heatmaps (JSON exportable).
