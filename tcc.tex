...
This monograph is organized as follows. Chapter~\ref{ch:literature} reviews the theoretical foundations and prior work in fault-tolerant UAV systems, multi-agent coordination, and the OODA loop framework, identifying the research gap this work addresses. Chapter~3 presents the system architecture, detailing the centralized OODA engine and distributed execution model. Chapter~4 describes the core algorithms and technical contributions, including the constraint-aware reallocation mechanism. Chapter~5 introduces the three mission scenarios used for validation. Chapter~6 details the experimental methodology and design. Chapter~7 presents the experimental results and statistical validation. Chapter~8 discusses limitations and future work. Chapter~9 concludes with a summary of contributions and their implications for autonomous UAV fleet operations.
...
To design, implement, and validate a constraint-aware fault-tolerant multi-agent coordination framework for Unmanned Aerial Vehicle systems, employing the OODA (Observe-Orient-Decide-Act) loop paradigm to enable mission continuity under operational constraints. This research addresses the methodological gap between theoretical fault tolerance mechanisms and deployable autonomous systems through explicit computational modeling of energy limitations, payload capacity, regulatory airspace boundaries, and human-in-the-loop decision authority.

\subsection{Specific Objectives}

\textbf{SO1: Hybrid OODA-FSM Architecture Design.}
To design and implement a centralized OODA Loop Engine integrated with a deterministic finite-state machine for continuous fleet state monitoring, enabling formally verifiable state transitions coupled with probabilistic failure identification while maintaining compliance with regulatory requirements for Beyond Visual Line of Sight (BVLOS) operations.

\textbf{SO2: Multi-Modal Real-Time Failure Detection.}
To develop and evaluate fault detection mechanisms operating at 2 Hz telemetry acquisition frequency, incorporating three complementary detection pathways: (i) communication timeout detection exceeding 1.5-second threshold, (ii) explicit fault code recognition from onboard diagnostic subsystems, and (iii) statistical anomaly detection for gradual degradation patterns, including abnormal battery discharge rates (>5\% per 30-second window), position discontinuities (>100m), and altitude constraint violations.

\textbf{SO3: Constraint-Aware Task Reallocation.}
To formalize and validate priority-based allocation algorithms that jointly optimize across multiple constraint dimensions: energy reserves (20\% safety margin), payload capacity limitations, temporal deadline satisfaction, and collision avoidance requirements (15m spatial separation buffer), employing sequential fail-fast constraint verification to guarantee physically realizable mission plans rather than theoretically optimal yet operationally infeasible allocations.

\textbf{SO4: Operator Escalation Framework.}
To establish formal decision rules distinguishing autonomously compensable failures from those necessitating human judgment, implementing graduated escalation policies: operator escalation for coverage recovery below 50\% or loss of high-priority tasks, partial reallocation for degradation within the 50--75\% recovery range, and autonomous full reallocation when coverage exceeds 75\%, thereby preserving operator authority over safety-critical determinations.

\textbf{SO5: Statistical Validation Across Mission Typologies.}
To conduct rigorous statistical validation (N=30 independent trials per experimental condition) across three distinct mission typologies---persistent surveillance, search-and-rescue operations, and payload delivery---quantifying coverage recovery percentage, adaptation latency, constraint violation frequency, and escalation appropriateness relative to no-adaptation baseline and greedy reallocation strategies.

\textbf{SO6: Real-Time Response Performance Characterization.}
To demonstrate OODA cycle execution within a 6-second end-to-end latency bound, with phase-specific targets (OBSERVE <500ms, ORIENT <500ms, DECIDE <1200ms, ACT <300ms), ensuring fault recovery computational overhead remains negligible relative to mission duration in time-critical operational contexts such as golden-hour search operations.

\textbf{SO7: Safety-Critical Constraint Satisfaction Guarantees.}
To empirically validate zero safety constraint violations across all experimental trials with respect to battery reserve thresholds, payload structural limitations, spatial separation requirements, and regulatory boundary compliance, thereby demonstrating that the proposed constraint-aware architecture maintains safety invariants without compromising mission coverage objectives.
...
This chapter examines the theoretical foundations and prior work relevant to fault-tolerant multi-UAV coordination under operational constraints. The review is organized thematically: Section~\ref{sec:lit-single-uav} addresses fault tolerance at the vehicle level, Section~\ref{sec:lit-multi-agent} examines multi-agent coordination architectures, Section~\ref{sec:lit-task-allocation} surveys task allocation algorithms, Section~\ref{sec:lit-coverage} discusses coverage control methods, Section~\ref{sec:lit-communication} reviews communication-aware coordination, and Section~\ref{sec:lit-ooda} traces the OODA loop from military theory to autonomous systems. Section~\ref{sec:lit-gap} synthesizes these threads to identify the specific research gap this work addresses.

\section{Fault Tolerance in Single Unmanned Aerial Vehicles}
\label{sec:lit-single-uav}

Vehicle-level fault tolerance has achieved substantial maturity through control-theoretic approaches. Mueller and D'Andrea \cite{mueller2014} demonstrated that quadrotors can maintain controlled flight despite the complete loss of one, two, or three propellers by exploiting the remaining actuator authority through control reallocation. Their experimental results showed stable hovering with a single propeller, albeit with reduced maneuverability and reliance on body rotation for attitude control.

Sun et al. \cite{sun2022} extended this capability to scenarios where the specific failed rotor is unknown a priori. Their model predictive control (MPC) formulation estimates failure configurations online and adapts control allocation accordingly, achieving stable recovery within 2.3 seconds of rotor failure onset in simulation trials.

\textbf{Critical Assessment:} These contributions establish that individual vehicles can survive actuator failures. However, both works assume (i) a ground-based operator or automated system to issue high-level commands post-recovery, (ii) no mission context requiring coordination with other vehicles, and (iii) unlimited time for recovery maneuvers. In fleet operations where failed vehicles hold assigned tasks and battery constraints limit mission duration, vehicle-level recovery alone proves insufficient---the mission layer must compensate for lost capacity.

\section{Multi-Agent Coordination Architectures}
\label{sec:lit-multi-agent}

Parker's \cite{parker1998} ALLIANCE architecture represents foundational work in fault-tolerant multi-robot cooperation. ALLIANCE employs behavior-based control with motivational dynamics: each robot maintains internal motivation levels for available tasks, with impatience parameters causing robots to assume tasks when teammates fail to perform adequately. Experimental validation on box-pushing and hazardous waste cleanup tasks demonstrated graceful degradation under robot failures.

\textbf{Limitations:} ALLIANCE assumes tasks are interchangeable among robots and does not model consumable resources. The motivational dynamics operate on implicit timeouts rather than explicit failure detection, introducing latency in failure response proportional to the impatience parameter settings.
...
\textbf{Critical Assessment:} Both works provide rigorous stability proofs but operate under assumptions that limit practical applicability: continuous communication, known system dynamics, and---critically---no resource constraints. Battery depletion, payload consumption, and regulatory boundaries do not appear in their problem formulations.
...
The Consensus-Based Bundle Algorithm (CBBA) of Choi et al. \cite{choi2009} enables decentralized task allocation through iterated auctions. Robots bid on task bundles based on marginal utility, with a consensus phase resolving conflicts. CBBA guarantees convergence to conflict-free assignments within $O(NM)$ communication rounds for $N$ robots and $M$ tasks, and demonstrates 50\% conflict resolution within three iterations in their experimental evaluation.

Market-based coordination, surveyed comprehensively by Dias et al. \cite{dias2006}, offers scalable allocation through economic mechanisms. Zlot and Stentz \cite{zlot2006} extended market approaches to complex tasks requiring decomposition, demonstrating allocation of exploration tasks among 80 simulated robots.

\textbf{Critical Assessment:} These algorithms optimize allocation efficiency but typically model robot capabilities as static. Dias et al. explicitly note that ``incorporating resource consumption and task deadlines remains an open challenge.'' CBBA's utility functions can theoretically encode battery constraints, but the original formulation and most subsequent applications assume fixed robot capabilities throughout the mission.

\section{Coverage Control and Spatial Coordination}
\label{sec:lit-coverage}

Voronoi-based coverage control, formalized by Cortés et al. \cite{cortes2004}, partitions the operational area among robots such that each robot is responsible for the region closest to its position. Their gradient-descent algorithm provably converges to locally optimal configurations that minimize a coverage cost functional.

Schwager et al. \cite{schwager2009} extended this to adaptive coverage, where the density function representing coverage importance is learned online from sensor measurements. Their decentralized algorithm achieves coverage optimization without global communication, requiring only local neighbor information.

Elmaliach et al. \cite{elmaliach2009} addressed multi-robot patrol under frequency constraints, ensuring each point receives visits at intervals not exceeding a specified bound. Their cyclic patrol strategies guarantee frequency constraints for convex environments and provide approximation bounds for non-convex cases.

\textbf{Critical Assessment:} Coverage algorithms assume robots operate continuously without resource depletion. When a robot must return to base for battery replacement or payload replenishment, the coverage guarantee breaks. None of these works address mid-mission robot loss or the resulting coverage gap quantification.

\section{Communication-Aware Multi-Vehicle Control}
\label{sec:lit-communication}

Communication constraints fundamentally affect multi-agent coordination feasibility. Abdessameud and Tayebi \cite{abdessameud2011} analyzed formation control for VTOL vehicles under communication delays, proving stability for delays below a computed threshold dependent on formation geometry and controller gains.

Izadi et al. \cite{izadi2009} developed decentralized receding horizon control (RHC) for cooperative vehicles with communication delays. Their approach partitions the fleet into subgroups with designated leaders, reducing communication requirements while maintaining coordination. Izadi et al. \cite{izadi2013} extended this to communication failures, demonstrating that hierarchical architectures with leader election protocols maintain formation stability despite intermittent link losses.

Beard et al. \cite{beard2006} presented one of the few complete systems integrating cooperative surveillance with communication constraints, deploying fixed-wing UAVs for decentralized target tracking. Their architecture explicitly addresses communication bandwidth limitations through prioritized information sharing.

\textbf{Critical Assessment:} These works establish that coordination remains possible under degraded communication but assume that communication eventually recovers or that the mission can tolerate permanent link losses. The interaction between communication failures and resource constraints---where delayed coordination may cause a UAV to exhaust its battery before receiving updated commands---remains unaddressed.

\section{The OODA Loop in Autonomous Systems}
\label{sec:lit-ooda}

Boyd's \cite{boyd1987} OODA loop---Observe, Orient, Decide, Act---originated from analysis of aerial combat, arguing that pilots who cycle through this loop faster gain decisive advantage. The framework emphasizes that ``Orient'' is the critical phase, where observations are filtered through experience, cultural traditions, genetic heritage, and previous analyses to create situational awareness.

Recent work has adapted OODA to autonomous systems. Bala et al. \cite{bala2025} applied OODA to cloudlet-based autonomous drones, mapping each phase to edge computing primitives: Observe to sensor data ingestion, Orient to context aggregation, Decide to inference at the edge, and Act to actuation commands. Their implementation demonstrated 340ms average decision latency for obstacle avoidance scenarios.

\textbf{Critical Assessment:} Bala et al. demonstrate OODA's applicability to single-drone decisions at the edge. However, their work does not address fleet-level coordination, failure recovery across multiple vehicles, or constraint satisfaction. The OODA phases are mapped to individual drone operations rather than fleet-wide situational awareness and resource management.
...
\begin{enumerate}
    ...
    \item \textbf{Multi-agent coordination} demonstrates scalable task allocation and coverage control but assumes static robot capabilities and unlimited resources.

    \item \textbf{Communication-aware control} addresses delays and link failures but does not integrate resource depletion or mission-level failure recovery.
\end{enumerate}
